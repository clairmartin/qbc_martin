...
quartz() # open a new figure window
plot(iris$Species, iris$Sepal.Width, # data to plot
     ylab="Sepal Width", # label axes
     col=c("hotpink", "plum", "cornflowerblue"), las=1, cex.axis=1.5,cex.lab=1.5) # make figure pretty
...
TukeyHSD(anova1) # examine each pair using a Tukey test POST HOC to check which are different from eachother
...
# is there a significant effect of species on the relationship between petal length and sepal width?
# set up a linear model with both petal length and species as effects:
